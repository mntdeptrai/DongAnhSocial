        ...
        <div class="tour-sidebar-content">
                ...
                <div class="glass-card" style="padding: 16px; border-radius: 16px; border-color: rgba(255,126,41,0.2); margin-bottom: 15px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                        <span style="font-size: 0.78rem; color: var(--text-muted);">
                            👤 Tạo bởi: <strong style="color: var(--primary);"><?php echo e($tour->user ? $tour->user->name : 'Thành viên cộng đồng'); ?></strong>
                        </span>
                        
                        <?php if($tour->shared_at): ?>
                    ...
                    <?php if(auth()->check() && (auth()->id() === $tour->user_id || session('user_role') === 'admin')): ?>
                        <div style="display: flex; gap: 8px; flex-wrap: wrap; align-items: center;">
                            <a href="/food-tour/<?php echo e($tour->slug); ?>/edit" class="btn-primary" style="flex: 1; text-align: center; text-decoration: none; padding: 8px 12px; border-radius: 8px; font-size: 0.78rem; font-weight: 700; display: flex; align-items: center; justify-content: center; gap: 4px;">
                                ✏️ Chỉnh sửa
                            </a>
                            
                            <form action="/food-tour/<?php echo e($tour->slug); ?>/share" method="POST" style="flex: 1.2; display: flex; margin: 0;">
                                <?php echo csrf_field(); ?>
                            ...
                            <form action="/food-tour/<?php echo e($tour->slug); ?>" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xoá lộ trình này không?')" style="display: inline-flex; margin: 0;">
                                <?php echo csrf_field(); ?>
            ...
            <div>
                <h4 style="font-weight: 800; color: var(--text-main); font-size: 0.95rem; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
                    <span>🗺️</span> <?php if($tour->mood === 'cooking'): ?> Lộ trình Trải nghiệm thực tế <?php else: ?> Các Chặng Dừng Chân <?php endif; ?>
                </h4>
                ...
                <div class="timeline-wrapper">
                    ...
                    <?php $__currentLoopData = $tour->stops; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $stop): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="timeline-item" id="stop-item-<?php echo e($index); ?>" data-index="<?php echo e($index); ?>" data-lat="<?php echo e(number_format($stop->eatery->latitude, 6, '.', '')); ?>" data-lng="<?php echo e(number_format($stop->eatery->longitude, 6, '.', '')); ?>" data-name="<?php echo e($stop->eatery->name); ?>">
                            ...
                            <div class="timeline-card" onclick="selectStop(<?php echo e($index); ?>)">
                                ...
                                <div class="timeline-card-story">
                                    ...
                                    <div style="display: flex; gap: 8px; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 10px; flex-wrap: wrap;">
                                        <!-- Check-in button -->
                                        <button class="checkin-action-btn" onclick="triggerCheckIn(event, <?php echo e($index); ?>)" style="flex: 1 1 100%; padding: 10px 12px; border-radius: 8px; background: <?php if($tour->mood === 'cooking'): ?> linear-gradient(135deg, #10b981 0%, #059669 100%) <?php else: ?> #0ea5e9 <?php endif; ?>; border: none; color: #ffffff; font-weight: 700; font-size: 0.78rem; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; transition: all 0.3s ease; margin-bottom: 4px;">
                                            <span class="check-icon-<?php echo e($index); ?>">✍️</span> <span class="check-text-<?php echo e($index); ?>">Đánh giá & Check-in</span>
                                        </button>
                                        
                                        <!-- Directions link -->
                                        <a id="tourStopDirectionsLink-<?php echo e($index); ?>" href="https://www.google.com/maps/dir/?api=1<?php echo e($index === 0 ? '' : '&origin='.number_format($tour->stops[$index - 1]->eatery->latitude, 6, '.', '').','.number_format($tour->stops[$index - 1]->eatery->longitude, 6, '.', '')); ?>&destination=<?php echo e(number_format($stop->eatery->latitude, 6, '.', '')); ?>,<?php echo e(number_format($stop->eatery->longitude, 6, '.', '')); ?>" target="_blank" rel="noopener noreferrer" class="btn-secondary" style="flex: 1; padding: 8px 12px; border-radius: 8px; font-size: 0.75rem; font-weight: 700; text-decoration: none; text-align: center; display: flex; align-items: center; justify-content: center; gap: 4px; border-color: <?php if($tour->mood === 'cooking'): ?> rgba(16, 185, 129, 0.4) <?php else: ?> rgba(14, 165, 233, 0.4) <?php endif; ?>; color: var(--primary);">
                                            🗺️ Chỉ đường
                                        </a>

                                        <!-- Quick show details link -->
                                        <a href="/dia-diem/<?php echo e($stop->eatery->slug); ?>" class="btn-secondary" style="flex: 1; padding: 8px 12px; border-radius: 8px; font-size: 0.75rem; font-weight: 600; text-decoration: none; text-align: center; display: flex; align-items: center; justify-content: center;">
                                            Xem chi tiết
                                        </a>
                                    </div>
                                </div>
            ...
            <div id="setupControlsPanel" style="margin-top: 20px; display: flex; flex-direction: column; gap: 12px;">
                <button class="btn-primary start-journey-btn" onclick="enterJourneyMode()" style="width: 100%; background: <?php if($tour->mood === 'cooking'): ?> linear-gradient(135deg, #10b981 0%, #059669 100%) <?php else: ?> var(--primary-grad) <?php endif; ?>; box-shadow: <?php if($tour->mood === 'cooking'): ?> 0 8px 24px rgba(16, 185, 129, 0.3) <?php else: ?> 0 8px 24px rgba(255, 126, 41, 0.3) <?php endif; ?>; padding: 12px; border-radius: 12px; font-weight: 800; border: none; cursor: pointer; color: white; display: flex; align-items: center; justify-content: center; gap: 6px;">
                    🚀 <?php if($tour->mood === 'cooking'): ?> Bắt đầu Trải nghiệm thực tế <?php else: ?> Bắt đầu Food Tour <?php endif; ?>
                </button>
...
<script>
    ...
    function exitAndGoHome() {
        ...
        exitJourneyMode();
        const targetUrl = '<?php echo e($tour->mood); ?>' === 'cooking' ? '/exp-corner' : '/food-tours';
...
<div id="completionModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.2); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); z-index: 10001; align-items: center; justify-content: center; padding: 15px 20px;">
    <div class="completion-scroll-card" style="width: 100%; max-width: 510px; padding: 22px 26px; border-radius: 24px; background: rgba(255, 255, 255, 0.96); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 2px solid rgba(255, 255, 255, 0.9); position: relative; box-shadow: 0 25px 60px rgba(15, 23, 42, 0.15); color: #0f172a; overflow: hidden;">
        ...
        <div id="certificateTicket" style="background: rgba(248, 250, 252, 0.7); border: 1.5px dashed var(--primary); border-radius: 14px; padding: 12px; margin-bottom: 12px;">
            ...
            <div id="shareCommunityBox" style="margin-bottom: 8px; padding: 10px 12px; border-radius: 10px; background: <?php if($tour->mood === 'cooking'): ?> linear-gradient(135deg, rgba(16,185,129,0.08), rgba(52,211,153,0.05)) <?php else: ?> linear-gradient(135deg, rgba(var(--primary-rgb), 0.08), rgba(6,182,212,0.05)) <?php endif; ?>; border: 1.5px solid <?php if($tour->mood === 'cooking'): ?> rgba(16,185,129,0.25) <?php else: ?> rgba(var(--primary-rgb), 0.25) <?php endif; ?>;">
                <label style="display: flex; align-items: flex-start; gap: 10px; cursor: pointer;">
                    <div style="position: relative; flex-shrink: 0; margin-top: 1px;">
                        <input type="checkbox" id="shareToCommunityToggle" style="width: 16px; height: 16px; accent-color: <?php if($tour->mood === 'cooking'): ?> #10b981 <?php else: ?> var(--primary) <?php endif; ?>; cursor: pointer;">
                    </div>
                    <div>
                        ...
                        <span style="font-size: 0.7rem; color: #64748b; display: block; margin-top: 2px; line-height: 1.4;">
                            Lộ trình sẽ hiển thị công khai trong <strong style="color: <?php if($tour->mood === 'cooking'): ?> #10b981 <?php else: ?> var(--primary) <?php endif; ?>;">72 giờ</strong> để người dùng khác có thể khám phá và trải nghiệm.
                        </span>
                    </div>
                </label>
            </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 6px;">
            ...
            <button onclick="saveJourneyDiary()" class="btn-primary" style="width: 100%; padding: 8px; border-radius: 8px; font-weight: 800; font-size: 0.8rem; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 4px; box-shadow: <?php if($tour->mood === 'cooking'): ?> 0 0 10px rgba(16, 185, 129, 0.25) <?php else: ?> 0 0 10px rgba(14, 165, 233, 0.25) <?php endif; ?>; background: <?php if($tour->mood === 'cooking'): ?> linear-gradient(135deg, #10b981 0%, #059669 100%) <?php else: ?> var(--primary-grad) <?php endif; ?>;">
...
<div id="communityDiariesModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.5); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); z-index: 10005; align-items: center; justify-content: center; animation: fadeIn 0.3s ease;">
    <div style="width: 90%; max-width: 480px; max-height: 85vh; display: flex; flex-direction: column; padding: 24px 20px; border-radius: 24px; background: rgba(26, 26, 38, 0.85); backdrop-filter: blur(25px); -webkit-backdrop-filter: blur(25px); border: 1.5px solid rgba(255, 255, 255, 0.1); box-shadow: 0 25px 60px rgba(0, 0, 0, 0.4); color: #ffffff; position: relative;">
        ...
        <h3 style="font-weight: 800; color: #ffffff; font-size: 1.25rem; margin: 0 0 16px 0; display: flex; align-items: center; gap: 10px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 16px;">
            📖 Nhật ký Cộng đồng
            <span style="font-size: 0.75rem; background: rgba(14,165,233,0.15); border: 1px solid rgba(14,165,233,0.3); color: #0ea5e9; padding: 3px 10px; border-radius: 20px; font-weight: 800;"><?php echo e(count($diaries)); ?> đánh giá</span>
        </h3>
...
<?php echo $__env->make('layouts.food-tour', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /www/wwwroot/donganhdiscovery.xadonganh.com/FOOD_MAP/resources/views/food-tours/show.blade.php ENDPATH**/ ?>