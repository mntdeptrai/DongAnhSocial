...
<div class="admin-sub-tabs">
    ...
    <?php if($eatery): ?>
        <?php if($eatery->category->slug === 'dong-anh-food-map'): ?>
            ...
            <button type="button" class="admin-sub-tab-btn" onclick="switchSubTab(event, 'tab-ocop-products')">
                🛍️ Sản phẩm OCOP & Đặc sản (<?php echo e(optional($eatery->relationLoaded('ocopProducts') ? $eatery->ocopProducts : collect())->count()); ?>)
            </button>
        <?php elseif($eatery->category->slug === 'dong-anh-market'): ?>
            <button type="button" class="admin-sub-tab-btn" onclick="switchSubTab(event, 'tab-ocop-products')">
                🛍️ Sản phẩm OCOP & Đặc sản (<?php echo e(optional($eatery->relationLoaded('ocopProducts') ? $eatery->ocopProducts : collect())->count()); ?>)
            </button>
        <?php elseif($eatery->category->slug === 'stay-in-dong-anh'): ?>
            <button type="button" class="admin-sub-tab-btn" onclick="switchSubTab(event, 'tab-rooms')">
                🛌 Quản lý phòng nghỉ (<?php echo e($eatery->rooms->count()); ?>)
            </button>
        <?php elseif($eatery->category->slug === 'wellness-care'): ?>
            <button type="button" class="admin-sub-tab-btn" onclick="switchSubTab(event, 'tab-wellness-services')">
                🩺 Dịch vụ chăm sóc & Spa (<?php echo e($eatery->wellnessServices->count()); ?>)
            </button>
        <?php elseif($eatery->category->slug === 'smart-education-map'): ?>
            <button type="button" class="admin-sub-tab-btn" onclick="switchSubTab(event, 'tab-education-programs')">
                🎓 Chương trình giáo dục (<?php echo e($eatery->educationPrograms->count()); ?>)
            </button>
        <?php elseif(in_array($eatery->category->slug, ['hanh-trinh-di-san', 'discover-dong-anh-community-culture-hub'])): ?>
            <button type="button" class="admin-sub-tab-btn" onclick="switchSubTab(event, 'tab-ocop-products')">
                🛍️ Sản phẩm OCOP & Đặc sản (<?php echo e(optional($eatery->relationLoaded('ocopProducts') ? $eatery->ocopProducts : collect())->count()); ?>)
            </button>
            <button type="button" class="admin-sub-tab-btn" onclick="switchSubTab(event, 'tab-cultural-activities')">
                🏛️ Hoạt động văn hóa & Trải nghiệm (<?php echo e(optional($eatery->relationLoaded('culturalActivities') ? $eatery->culturalActivities : collect())->count()); ?>)
            </button>
        <?php endif; ?>

        <button type="button" class="admin-sub-tab-btn" onclick="switchSubTab(event, 'tab-photos')">
            🖼️ Ảnh thực tế (<?php echo e(optional($eatery?->relationLoaded('photos') ? $eatery->photos : collect())->count()); ?>)
        </button>
        <button type="button" class="admin-sub-tab-btn" onclick="switchSubTab(event, 'tab-videos')">
            3. Video Review của quán (<?php echo e(optional($eatery->relationLoaded('reviewVideos') ? $eatery->reviewVideos : collect())->count()); ?>)
        </button>
...
<div id="tab-info" class="admin-tab-section" style="display: block;">
    <div class="admin-card">
        ...
        <form action="<?php echo e($eatery ? '/admin/eateries/' . $eatery->slug : '/admin/eateries'); ?>" method="POST" enctype="multipart/form-data">
            <?php echo csrf_field(); ?>
            <?php if($eatery): ?>
                ...
                <div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                        <div class="admin-form-group">
                            ...
                            <select name="category_id" class="admin-form-input" required>
                                ...
                                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    ...
                                    ?>
                                    <option value="<?php echo e($cat->id); ?>" data-slug="<?php echo e($cat->slug); ?>" <?php echo e(old('category_id', $eatery ? $eatery->category_id : '') == $cat->id ? 'selected' : ''); ?>>
                                        <?php echo e($displayIcon); ?> <?php echo e($displayNameVi); ?> (<?php echo e($displayNameEn); ?>)
                                    </option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                        <div class="admin-form-group">
                            ...
                            <div class="custom-select-wrapper" style="position: relative;">
                                <input type="text" id="communeSearch" name="commune_name" class="admin-form-input" placeholder="🔍 Nhập tên Thôn / Tổ dân phố để tìm..." value="<?php echo e(old('commune_name', $eatery && $eatery->commune ? $eatery->commune->name : '')); ?>" autocomplete="off" required style="padding-right: 32px;">
                                ...
                                <div id="communeDropdown" class="custom-dropdown-list glass-panel" style="display: none; position: absolute; top: 100%; left: 0; right: 0; max-height: 250px; overflow-y: auto; z-index: 1000; background: var(--admin-card-bg); border: 1px solid var(--admin-border); border-radius: 8px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); margin-top: 4px; backdrop-filter: blur(8px);">
                                    <?php $__currentLoopData = $communes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $com): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div class="dropdown-item-com" data-id="<?php echo e($com->id); ?>" data-name="<?php echo e($com->name); ?>" style="padding: 10px 16px; cursor: pointer; border-bottom: 1px solid rgba(255,255,255,0.05); font-size: 0.9rem; color: var(--admin-text-muted); transition: all 0.2s;">
                                            📍 <?php echo e($com->name); ?>

                                        </div>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </div>
                                <input type="hidden" name="commune_id" id="communeIdHidden" value="<?php echo e(old('commune_id', $eatery ? $eatery->commune_id : '')); ?>">
                            </div>
                            <style>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                        <div class="admin-form-group" id="priceRangeGroup">
                            ...
                            <input type="text" name="price_range" id="priceRangeInput" class="admin-form-input" placeholder="Ví dụ: 30.000đ - 80.000đ" value="<?php echo e(old('price_range', $eatery ? $eatery->price_range : '')); ?>">
                        </div>
                ...
                <div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                        <div class="admin-form-group">
                            ...
                            <input type="number" step="any" name="latitude" id="latInput" class="admin-form-input" required placeholder="Ví dụ: 21.1182" value="<?php echo e(old('latitude', $eatery ? number_format($eatery->latitude, 6, '.', '') : '')); ?>">
                        </div>
                        <div class="admin-form-group">
                            <label class="admin-form-label">Kinh độ (Longitude) <span style="color: var(--admin-danger);">*</span></label>
                            <input type="number" step="any" name="longitude" id="lngInput" class="admin-form-input" required placeholder="Ví dụ: 105.8394" value="<?php echo e(old('longitude', $eatery ? number_format($eatery->longitude, 6, '.', '') : '')); ?>">
                        </div>
                    </div>
                        ...
                        <?php $__currentLoopData = $eatery->dishes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dish): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="admin-dish-item" style="display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 14px; border: 1.5px solid var(--admin-border); border-radius: 12px; background-color: #ffffff;">
                                ...
                                <div style="display: flex; align-items: center; gap: 6px;">
                                    ...
                                    <!-- Edit Button -->
                                    <button type="button" class="btn-admin btn-admin-accent" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;" data-dish="<?php echo e(json_encode($dish)); ?>" onclick="openEditDishModal(this)">
                                        ✏️ Sửa
                                    </button>
...
<?php if($eatery && in_array($eatery->category->slug, ['dong-anh-market', 'dong-anh-food-map', 'hanh-trinh-di-san', 'discover-dong-anh-community-culture-hub'])): ?>
<!-- ==========================================================================
...
<div id="tab-ocop-products" class="admin-tab-section" style="display: none;">
    <div class="admin-card">
        ...
        <div style="padding: 20px;">
            <h3 style="font-size: 0.95rem; font-weight: 700; margin-bottom: 16px; color: var(--admin-text-main);">
                📋 Sản phẩm hiện tại (<?php echo e(optional($eatery->relationLoaded('ocopProducts') ? $eatery->ocopProducts : collect())->count()); ?>)
            </h3>

            <?php $eateryOcopProducts = $eatery->relationLoaded('ocopProducts') ? $eatery->ocopProducts : collect(); ?>
            <?php if($eateryOcopProducts->count() > 0): ?>
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 16px;">
                    <?php $__currentLoopData = $eateryOcopProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="admin-dish-item" style="display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 14px; border: 1.5px solid var(--admin-border); border-radius: 12px; background-color: #ffffff;">
                            <div style="display: flex; align-items: center; gap: 14px; flex: 1; min-width: 0;">
                                <img src="<?php echo e($product->image_path ?: asset('images/ocop-placeholder.png')); ?>" style="width: 56px; height: 56px; border-radius: 10px; object-fit: cover;">
                                <div style="flex: 1; min-width: 0;">
                                    <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                                        <h4 style="margin: 0; font-weight: 700; font-size: 0.92rem; color: var(--admin-text-main);"><?php echo e($product->name); ?></h4>
                                        <?php if($product->star_rating): ?>
                                            <span class="admin-badge admin-badge-primary" style="font-size: 0.65rem; padding: 2px 6px;"><?php echo e($product->star_rating); ?></span>
                                        <?php endif; ?>
                                    </div>
                                    <span style="font-size: 0.76rem; color: var(--admin-text-muted); display: block; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?php echo e($product->description ?: 'Chưa có mô tả'); ?></span>
                                    <span style="font-size: 0.85rem; font-weight: 800; color: var(--admin-success); display: block; margin-top: 1px;">
                                        <?php echo e($product->price ? number_format($product->price, 0, ',', '.') . 'đ' : 'Liên hệ'); ?>
                            ...
                            <div style="display: flex; align-items: center; gap: 6px;">
                                <!-- Edit Button -->
                                <button type="button" class="btn-admin btn-admin-accent" data-product="<?php echo e(json_encode($product)); ?>" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;" onclick="openEditOcopProductModal(this)">
                                    ✏️ Sửa
                                </button>

                                <!-- Delete Button -->
                                <form action="/admin/ocop-products/<?php echo e($product->id); ?>" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa sản phẩm này?')" style="display: inline; margin: 0;">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('DELETE'); ?>
                                    <button type="submit" class="btn-admin btn-admin-danger" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;">
                                        🗑️ Xóa
...
<?php if($eatery && $eatery->category->slug === 'stay-in-dong-anh'): ?>
<!-- ==========================================================================
...
<div id="tab-rooms" class="admin-tab-section" style="display: none;">
    <div class="admin-card">
        ...
        <div class="admin-split-layout">
            ...
            <div>
                <h3 style="font-size: 0.95rem; font-weight: 700; margin-bottom: 16px; color: var(--admin-text-main);">
                    📋 Danh sách phòng hiện có (<?php echo e($eatery->rooms->count()); ?>)
                </h3>

                <?php if($eatery->rooms->count() > 0): ?>
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        <?php $__currentLoopData = $eatery->rooms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $room): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="admin-dish-item" style="display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 14px; border: 1.5px solid var(--admin-border); border-radius: 12px; background-color: #ffffff;">
                                <div style="display: flex; align-items: center; gap: 14px; flex: 1; min-width: 0;">
                                    <img src="<?php echo e($room->image_path ?: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=80&q=80'); ?>" style="width: 56px; height: 56px; border-radius: 10px; object-fit: cover;">
                                    <div style="flex: 1; min-width: 0;">
                                        <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                                            <h4 style="margin: 0; font-weight: 700; font-size: 0.92rem; color: var(--admin-text-main);"><?php echo e($room->name); ?></h4>
                                            <?php if($room->bed_type): ?>
                                                <span class="admin-badge admin-badge-success" style="font-size: 0.65rem; padding: 2px 6px;">🛏️ <?php echo e($room->bed_type); ?></span>
                                            <?php endif; ?>
                                            <span class="admin-badge admin-badge-primary" style="font-size: 0.65rem; padding: 2px 6px;">👥 Tối đa: <?php echo e($room->capacity); ?> người</span>
                                        </div>
                                        <span style="font-size: 0.76rem; color: var(--admin-text-muted); display: block; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?php echo e($room->description ?: 'Chưa có mô tả'); ?></span>
                                        <span style="font-size: 0.85rem; font-weight: 800; color: var(--admin-success); display: block; margin-top: 1px;"><?php echo e(number_format($room->price, 0, ',', '.')); ?>đ / đêm</span>
                                    </div>
                                </div>
                                
                                <div style="display: flex; align-items: center; gap: 6px;">
                                    <!-- Edit Button -->
                                    <button type="button" class="btn-admin btn-admin-accent" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;" data-room="<?php echo e(json_encode($room)); ?>" onclick="openEditRoomModal(this)">
                                        ✏️ Sửa
                                    </button>

                                    <!-- Delete Button -->
                                    <form action="/admin/rooms/<?php echo e($room->id); ?>" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa phòng này?')" style="display: inline; margin: 0;">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('DELETE'); ?>
                                        <button type="submit" class="btn-admin btn-admin-danger" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;">
                                            🗑️ Xóa
...
<?php if($eatery && $eatery->category->slug === 'wellness-care'): ?>
<!-- ==========================================================================
...
<div id="tab-wellness-services" class="admin-tab-section" style="display: none;">
    <div class="admin-card">
        ...
        <div class="admin-split-layout">
            ...
            <div>
                <h3 style="font-size: 0.95rem; font-weight: 700; margin-bottom: 16px; color: var(--admin-text-main);">
                    📋 Danh sách dịch vụ hiện tại (<?php echo e($eatery->wellnessServices->count()); ?>)
                </h3>

                <?php if($eatery->wellnessServices->count() > 0): ?>
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        <?php $__currentLoopData = $eatery->wellnessServices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="admin-dish-item" style="display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 14px; border: 1.5px solid var(--admin-border); border-radius: 12px; background-color: #ffffff;">
                                <div style="display: flex; align-items: center; gap: 14px; flex: 1; min-width: 0;">
                                    <img src="<?php echo e($service->image_path ?: 'https://images.unsplash.com/photo-1540555700478-4be289fbecef?auto=format&fit=crop&w=80&q=80'); ?>" style="width: 56px; height: 56px; border-radius: 10px; object-fit: cover;">
                                    <div style="flex: 1; min-width: 0;">
                                        <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                                            <h4 style="margin: 0; font-weight: 700; font-size: 0.92rem; color: var(--admin-text-main);"><?php echo e($service->name); ?></h4>
                                            <?php if($service->duration): ?>
                                                <span class="admin-badge admin-badge-success" style="font-size: 0.65rem; padding: 2px 6px;">⏱️ <?php echo e($service->duration); ?></span>
                                            <?php endif; ?>
                                        </div>
                                        <span style="font-size: 0.76rem; color: var(--admin-text-muted); display: block; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?php echo e($service->description ?: 'Chưa có mô tả'); ?></span>
                                        <span style="font-size: 0.85rem; font-weight: 800; color: var(--admin-success); display: block; margin-top: 1px;">
                                            <?php echo e($service->price ? number_format($service->price, 0, ',', '.') . 'đ' : 'Liên hệ/Tư vấn'); ?>
                                ...
                                <div style="display: flex; align-items: center; gap: 6px;">
                                    <!-- Edit Button -->
                                    <button type="button" class="btn-admin btn-admin-accent" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;" data-service="<?php echo e(json_encode($service)); ?>" onclick="openEditWellnessServiceModal(this)">
                                        ✏️ Sửa
                                    </button>

                                    <!-- Delete Button -->
                                    <form action="/admin/wellness-services/<?php echo e($service->id); ?>" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa dịch vụ này?')" style="display: inline; margin: 0;">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('DELETE'); ?>
                                        <button type="submit" class="btn-admin btn-admin-danger" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;">
                                            🗑️ Xóa
...
<?php if($eatery && $eatery->category->slug === 'smart-education-map'): ?>
<!-- ==========================================================================
...
<div id="tab-education-programs" class="admin-tab-section" style="display: none;">
    <div class="admin-card">
        ...
        <div class="admin-split-layout">
            ...
            <div>
                <h3 style="font-size: 0.95rem; font-weight: 700; margin-bottom: 16px; color: var(--admin-text-main);">
                    📋 Các chương trình hiện tại (<?php echo e($eatery->educationPrograms->count()); ?>)
                </h3>

                <?php if($eatery->educationPrograms->count() > 0): ?>
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        <?php $__currentLoopData = $eatery->educationPrograms; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $program): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="admin-dish-item" style="display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 14px; border: 1.5px solid var(--admin-border); border-radius: 12px; background-color: #ffffff;">
                                <div style="display: flex; align-items: center; gap: 14px; flex: 1; min-width: 0;">
                                    <img src="<?php echo e($program->image_path ?: 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?auto=format&fit=crop&w=80&q=80'); ?>" style="width: 56px; height: 56px; border-radius: 10px; object-fit: cover;">
                                    <div style="flex: 1; min-width: 0;">
                                        <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                                            <h4 style="margin: 0; font-weight: 700; font-size: 0.92rem; color: var(--admin-text-main);"><?php echo e($program->name); ?></h4>
                                            <?php if($program->duration): ?>
                                                <span class="admin-badge admin-badge-success" style="font-size: 0.65rem; padding: 2px 6px;">⏱️ <?php echo e($program->duration); ?></span>
                                            <?php endif; ?>
                                        </div>
                                        <span style="font-size: 0.76rem; color: var(--admin-text-muted); display: block; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?php echo e($program->description ?: 'Chưa có mô tả'); ?></span>
                                        <span style="font-size: 0.85rem; font-weight: 800; color: var(--admin-accent); display: block; margin-top: 1px;">💵 <?php echo e($program->tuition_fee ?: 'Liên hệ tuyển sinh'); ?></span>
                                    </div>
                                </div>
                                
                                <div style="display: flex; align-items: center; gap: 6px;">
                                    <!-- Edit Button -->
                                    <button type="button" class="btn-admin btn-admin-accent" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;" data-program="<?php echo e(json_encode($program)); ?>" onclick="openEditEducationProgramModal(this)">
                                        ✏️ Sửa
                                    </button>

                                    <!-- Delete Button -->
                                    <form action="/admin/education-programs/<?php echo e($program->id); ?>" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa chương trình đào tạo này?')" style="display: inline; margin: 0;">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('DELETE'); ?>
                                        <button type="submit" class="btn-admin btn-admin-danger" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;">
                                            🗑️ Xóa
...
<?php if($eatery && in_array($eatery->category->slug, ['hanh-trinh-di-san', 'discover-dong-anh-community-culture-hub'])): ?>
<!-- ==========================================================================
...
<div id="tab-cultural-activities" class="admin-tab-section" style="display: none;">
    <div class="admin-card">
        ...
        <div class="admin-split-layout">
            ...
            <div>
                <?php $culturalActivities = $eatery->relationLoaded('culturalActivities') ? $eatery->culturalActivities : collect(); ?>
                <h3 style="font-size: 0.95rem; font-weight: 700; margin-bottom: 16px; color: var(--admin-text-main);">
                    📋 Danh sách hoạt động (<?php echo e($culturalActivities->count()); ?>)
                </h3>

                <?php if($culturalActivities->count() > 0): ?>
                    <div style="display: flex; flex-direction: column; gap: 16px;">
                        <?php $__currentLoopData = $culturalActivities; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $act): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="admin-dish-item" style="display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; padding: 14px; border: 1.5px solid var(--admin-border); border-radius: 12px; background-color: #ffffff;">
                                <div style="display: flex; gap: 14px; flex: 1; min-width: 0;">
                                    <img src="<?php echo e($act->image_path ?: 'https://images.unsplash.com/photo-1596422846543-75c6fc18a523?auto=format&fit=crop&w=80&q=80'); ?>" class="admin-dish-img" style="width: 72px; height: 72px; border-radius: 10px; object-fit: cover; flex-shrink: 0;">
                                    <div style="min-width: 0; flex: 1;">
                                        <div style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                                            <h4 class="admin-dish-title" style="margin: 0; font-weight: 700; font-size: 0.92rem; color: var(--admin-text-main);"><?php echo e($act->name); ?></h4>
                                            
                                            <?php if($act->type === 'experience'): ?>
                                        ...
                                        <?php else: ?>
                                            <span class="admin-dish-price" style="font-size: 0.82rem; font-weight: 700; color: var(--admin-text-muted); display: block; margin-top: 4px;">
                                                Theo yêu cầu dâng lễ / <?php echo e($act->unit); ?>

                                            </span>
                                        <?php endif; ?>

                                        <?php if($act->discount_note && $act->discount_note !== 'null'): ?>
                                            <div style="font-size: 0.74rem; color: #dd6b20; font-weight: 600; margin-top: 3px; display: flex; align-items: center; gap: 4px;">
                                                🏷️ <?php echo e($act->discount_note); ?>

                                            </div>
                                        <?php endif; ?>

                                        <?php if($act->description && $act->description !== 'null'): ?>
                                ...
                                <div style="display: flex; gap: 6px; align-items: center; flex-shrink: 0;">
                                    <button type="button" class="btn-admin btn-admin-accent" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;" data-activity="<?php echo e(json_encode($act)); ?>" onclick="openEditCulturalActivityModal(this)">
                                        ✏️ Sửa
                                    </button>
                                    
                                    <form action="/admin/cultural-activities/<?php echo e($act->id); ?>" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa hoạt động này?')" style="display: inline; margin: 0;">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('DELETE'); ?>
                                        <button type="submit" class="btn-admin btn-admin-danger" style="padding: 6px 10px; font-size: 0.72rem; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px;">
                                            ✕ Xóa
            ...
            <div>
                <div style="padding: 20px; border: 1.5px solid var(--admin-border); border-radius: 12px; background-color: #f8fafc; position: sticky; top: 20px;">
                    ...
                    <form action="/admin/cultural-activities" method="POST" enctype="multipart/form-data">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="eatery_id" value="<?php echo e($eatery->id); ?>">
                        <input type="hidden" name="eatery_slug" value="<?php echo e($eatery->slug); ?>">
...
<div id="tab-photos" class="admin-tab-section" style="display: none;">
    <div class="admin-card">
        <div class="admin-card-header">
            ...
            <span style="font-size: 0.8rem; color: var(--admin-text-muted);"><?php echo e($eatery ? $eatery->photos->count() : 0); ?> ảnh đã tải lên</span>
        </div>

        
        <?php if($eatery && $eatery->photos->count() > 0): ?>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 14px; padding: 20px; margin-bottom: 8px;">
            <?php $__currentLoopData = $eatery->photos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $photo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div style="position: relative; border-radius: 12px; overflow: hidden; aspect-ratio: 1; background: var(--admin-bg); border: 1.5px solid var(--admin-border); box-shadow: 0 2px 8px rgba(0,0,0,0.06);">
                <img src="<?php echo e($photo->image_path); ?>" alt="<?php echo e($photo->caption ?: 'Ảnh cơ sở'); ?>"
                     style="width: 100%; height: 100%; object-fit: cover; display: block;"
                     onerror="this.src='<?php echo e(asset('images/ocop-placeholder.png')); ?>'">
                ...
                <div style="position: absolute; bottom: 0; left: 0; right: 0; padding: 8px; color: white; font-size: 0.72rem; opacity: 0; transition: opacity 0.2s;" class="photo-caption-overlay"><?php echo e($photo->caption); ?></div>
                <?php endif; ?>
                <form action="<?php echo e(route('admin.eatery-photo.destroy', $photo->id)); ?>" method="POST"
    ...
    <div class="admin-card" style="margin-top: 20px;">
        ...
        <form action="<?php echo e(route('admin.eatery-photo.store')); ?>" method="POST" enctype="multipart/form-data" style="padding: 20px;">
            <?php echo csrf_field(); ?>
            <input type="hidden" name="eatery_id" value="<?php echo e($eatery?->id); ?>">
...
<div id="tab-videos" class="admin-tab-section" style="display: none;">
    <div class="admin-card">
        ...
        <div class="admin-split-layout">
            ...
            <div>
                <h3 style="font-size: 0.95rem; font-weight: 700; margin-bottom: 16px; color: var(--admin-text-main);">
                    📋 Danh sách video hiện tại (<?php echo e(optional($eatery->relationLoaded('reviewVideos') ? $eatery->reviewVideos : collect())->count()); ?>)
                </h3>

                <?php $eateryReviewVideos = $eatery->relationLoaded('reviewVideos') ? $eatery->reviewVideos : collect(); ?>
                <?php if($eateryReviewVideos->count() > 0): ?>
                    <div class="admin-table-container">
                        <table class="admin-data-table">
                            ...
                            <tbody>
                                <?php $__currentLoopData = $eateryReviewVideos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $vid): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr>
                                        <td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <div style="margin-top: 8px; padding: 10px 14px; background-color: #f8fafc; border-left: 3px solid var(--admin-primary); border-radius: 6px; font-size: 0.82rem;">
                                            ...
                                            <p style="margin: 0; color: var(--admin-text-main); font-style: italic;"><?php echo e($rev->seller_reply); ?></p>
                                        </div>
                                        <?php endif; ?>

                                        <!-- Form phản hồi (Ẩn mặc định) -->
                                        <?php if(session('user_role') === 'seller'): ?>
                                        <div id="reply-form-<?php echo e($rev->id); ?>" style="display: none; margin-top: 10px; padding: 12px; border: 1.5px solid var(--admin-border); border-radius: 8px; background-color: #f8fafc;">
                                            <form action="/admin/reviews/<?php echo e($rev->id); ?>/reply" method="POST">
                                                <?php echo csrf_field(); ?>
                                                <div class="admin-form-group" style="margin-bottom: 8px;">
                                                    ...
                                                    <textarea name="seller_reply" rows="2" class="admin-form-input" style="font-size: 0.82rem; padding: 6px 10px;" placeholder="Cảm ơn bạn đã phản hồi tốt về quán. Chúng tôi sẽ cố gắng phát huy tốt hơn nữa..."><?php echo e($rev->seller_reply); ?></textarea>
                                                </div>
                                                <div style="display: flex; gap: 6px; justify-content: flex-end;">
                                                    <button type="button" class="btn-admin btn-admin-secondary" style="padding: 4px 10px; font-size: 0.72rem; border-radius: 6px;" onclick="toggleReplyForm(<?php echo e($rev->id); ?>)">Hủy</button>
                                    ...
                                    <td style="text-align: center;">
                                        <button type="button" class="btn-admin <?php echo e($rev->seller_reply ? 'btn-admin-secondary' : 'btn-admin-accent'); ?>" style="padding: 4px 8px; font-size: 0.72rem; border-radius: 4px; font-weight: 700; cursor: pointer;" onclick="toggleReplyForm(<?php echo e($rev->id); ?>)">
                                            <?php echo e($rev->seller_reply ? '✏️ Sửa PH' : '💬 Phản hồi'); ?>
...
<script>
    ...
    const purchaseInvoices = <?php echo json_encode($mappedInvoices, 15, 512) ?>;
    ...
    const initLat = <?php echo e($eatery ? number_format($eatery->latitude, 6, '.', '') : 21.1182); ?>;
    const initLng = <?php echo e($eatery ? number_format($eatery->longitude, 6, '.', '') : 105.8394); ?>;
    let pickerMap;
    let marker;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /www/wwwroot/donganhdiscovery.xadonganh.com/FOOD_MAP/resources/views/admin/eatery-form.blade.php ENDPATH**/ ?>