                            ...
                            <article class="checkin-post-card glass-panel" data-checkin-id="<?php echo e($checkin->id); ?>" data-user-id="<?php echo e($checkin->user_id ?? ''); ?>" style="margin-bottom: 20px;">
                                ...
                                <div class="reactions-bar">
                                    ...
                                    <div style="display:flex; gap:8px;">
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($checkin->id); ?>, 'checkin', '❤️', event)">❤️</button>
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($checkin->id); ?>, 'checkin', '🔥', event)">🔥</button>
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($checkin->id); ?>, 'checkin', '👍', event)">👍</button>
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($checkin->id); ?>, 'checkin', '😂', event)">😂</button>
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($checkin->id); ?>, 'checkin', '😍', event)">😍</button>
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($checkin->id); ?>, 'checkin', '🤤', event)">🤤</button>
                        ...
                        <?php $__currentLoopData = $diaries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $diary): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <article class="checkin-post-card glass-panel" data-diary-id="<?php echo e($diary->id); ?>" data-user-id="<?php echo e($diary->user_id ?? ''); ?>" style="margin-bottom: 20px;">
                                ...
                                <div class="reactions-bar">
                                    <span style="font-size:0.8rem; font-weight:800; color:var(--text-muted);">Thả cảm xúc:</span>
                                    <div style="display:flex; gap:8px;">
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($diary->id); ?>, 'diary', '❤️', event)">❤️</button>
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($diary->id); ?>, 'diary', '🔥', event)">🔥</button>
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($diary->id); ?>, 'diary', '👍', event)">👍</button>
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($diary->id); ?>, 'diary', '😂', event)">😂</button>
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($diary->id); ?>, 'diary', '😍', event)">😍</button>
                                        <button type="button" class="react-btn" onclick="sendReaction(<?php echo e($diary->id); ?>, 'diary', '🤤', event)">🤤</button>
...
<script>
    ...
    function renderLocketEateryDropdown(results) {
        const dropdown = document.getElementById('locketEateryDropdown');
        if (!results.length) {
            dropdown.innerHTML = `<div style="padding: 10px 12px; color: var(--text-muted); font-size: 0.8rem; text-align: center;">Không tìm thấy quán</div>`;
            dropdown.style.display = 'block';
            return;
        }
        dropdown.innerHTML = results.map(e => `
            <div onclick="selectLocketEatery(${e.id}, '${e.name.replace(/'/g, "\\'").replace(/"/g, '&quot;')}', '${(e.category||'').replace(/'/g,"\\'")}', '${(e.commune||'').replace(/'/g,"\\'")}', '${e.image ?? ''}')"
    ...
    function selectLocketEatery(id, name, category, commune, image) {
                ...
                fetch(`/api/eateries/nearby?lat=${lat}&lng=${lng}&radius=5`)
                    ...
                    .then(results => {
                        ...
                        if (results && results.length > 0) {
                            ...
                            selectLocketEatery(nearest.id, nearest.name, nearest.category, nearest.commune, nearest.image);
                        } else {
                            alert('Không tìm thấy quán nào gần bạn trong bán kính 5km.');
                        }
                    })
                    .catch(() => {
                        gpsBtn.innerHTML = origText;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/mnt/Downloads/FOOD_MAP/resources/views/checkin.blade.php ENDPATH**/ ?>