<?php $__env->startSection('title', 'Cộng đồng Check-in & Nhật ký hành trình - DongAnh Map Discovery'); ?>
<?php $__env->startSection('meta_description', 'Khám phá các hình ảnh check-in thực tế, đánh giá chi tiết và trải nghiệm du lịch ẩm thực Đông Anh từ cộng đồng.'); ?>
...
<div class="checkin-feed-container">
    <div style="width: 100%; display: flex; flex-direction: column; z-index: 2;">
        ...
        <div class="locket-widget-wrapper">
            <div class="locket-widget-card glass-panel">
                ...
                <div id="locketDetailsForm" style="display: none;" class="locket-details-form">
                    <form action="<?php echo e(route('checkin.store')); ?>" method="POST" enctype="multipart/form-data" id="locketCheckinForm">
                        <?php echo csrf_field(); ?>
                        ...
                        <div class="form-group">
                            ...
                            <div class="locket-stars-row">
                                <?php for($i = 1; $i <= 5; $i++): ?>
                                    <span class="locket-star" data-val="<?php echo e($i); ?>" onclick="setLocketRating(<?php echo e($i); ?>)">⭐</span>
                                <?php endfor; ?>
                            </div>
        ...
        <?php if($standaloneCheckins->isNotEmpty()): ?>
            <?php
            ...
            <div style="margin-bottom: 24px;">
                ...
                <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                    <?php $__currentLoopData = $hotPlaces; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $eateryId => $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $hotEatery = $group->first()->eatery; ?>
                        <?php if($hotEatery): ?>
                            <a href="/dia-diem/<?php echo e($hotEatery->slug); ?>"
                                ...
                                <img src="<?php echo e($hotEatery->image_path ?? 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=60&q=60'); ?>"
                                    alt="<?php echo e($hotEatery->name); ?>" style="width: 28px; height: 28px; border-radius: 50%; object-fit: cover; border: 1.5px solid var(--border-glow);">
                                <span style="font-size: 0.85rem; font-weight: 700; color: var(--text-main);"><?php echo e($hotEatery->name); ?></span>
                                <span style="font-size: 0.72rem; font-weight: 600; color: var(--primary); background: rgba(var(--primary-rgb), 0.1); padding: 2px 7px; border-radius: 20px;"><?php echo e($group->count()); ?> 📸</span>
                            </a>
                        <?php endif; ?>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
        ...
        <div style="width: 100%;">
                ...
                <div class="feed-tabs" style="display: flex; gap: 12px; margin-bottom: 24px; border-bottom: 1.5px solid var(--border-glow); padding-bottom: 12px;">
                    <button class="feed-tab-btn active" onclick="switchFeedTab('all', this)" style="background: rgba(14, 165, 233, 0.1); border: none; font-size: 0.95rem; font-weight: 800; color: var(--primary); cursor: pointer; padding: 6px 14px; border-radius: 20px; transition: all 0.3s; display: flex; align-items: center; gap: 8px;">
                        🌍 Cộng đồng Check-in <span class="tab-badge" style="background: rgba(14, 165, 233, 0.15); color: var(--primary); font-size: 0.72rem; padding: 2px 8px; border-radius: 20px;"><?php echo e($standaloneCheckins->count() + $diaries->count()); ?></span>
                    </button>
                    <button class="feed-tab-btn" onclick="switchFeedTab('my', this)" style="background: none; border: none; font-size: 0.95rem; font-weight: 800; color: var(--text-muted); cursor: pointer; padding: 6px 14px; border-radius: 20px; transition: all 0.3s; display: flex; align-items: center; gap: 8px;">
                        👤 Bài viết của tôi <span class="tab-badge" style="background: rgba(255, 255, 255, 0.05); color: var(--text-muted); font-size: 0.72rem; padding: 2px 8px; border-radius: 20px;"><?php echo e($standaloneCheckins->where('user_id', auth()->id())->count() + $diaries->where('user_id', auth()->id())->count()); ?></span>
                    </button>
                </div>
                <?php endif; ?>
                ...
                <?php if($standaloneCheckins->isNotEmpty()): ?>
                    <div id="standaloneCheckinsSection" style="margin-bottom: 8px;">
                        ...
                        <?php $__currentLoopData = $standaloneCheckins; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $checkin): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <article class="checkin-post-card glass-panel" data-checkin-id="<?php echo e($checkin->id); ?>" data-user-id="<?php echo e($checkin->user_id ?? ''); ?>" style="margin-bottom: 20px;">
                                <!-- Post Author Header -->
                                <div class="post-header">
                                    <div class="post-author">
                                        <div class="author-avatar">
                                            <?php echo e($checkin->user ? mb_substr($checkin->user->name, 0, 1, 'UTF-8') : '👤'); ?>

                                        </div>
                                        <div class="author-meta">
                                            <span class="author-name"><?php echo e($checkin->display_name); ?></span>
                                            <span class="author-role-badge <?php echo e(($checkin->user && $checkin->user->role === 'admin') ? 'role-admin' : 'role-user'); ?>">
                                                <?php echo e(($checkin->user && $checkin->user->role === 'admin') ? 'Admin' : 'Thực khách'); ?>
                                    ...
                                    <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 6px;">
                                        <div class="rating-stars-container">
                                            <?php for($i = 1; $i <= 5; $i++): ?>
                                                <span class="star-icon <?php echo e($i <= $checkin->rating ? 'filled' : 'empty'); ?>">★</span>
                                            <?php endfor; ?>
                                        </div>
                                        <span class="post-time">📅 <?php echo e($checkin->created_at->diffForHumans()); ?> (<?php echo e($checkin->created_at->format('d/m/Y H:i')); ?>)</span>
                                    </div>
                                </div>

                                <!-- Eatery badge -->
                                <?php if($checkin->eatery): ?>
                                    <a href="/dia-diem/<?php echo e($checkin->eatery->slug); ?>" class="post-tour-link">
                                        📍 <?php echo e($checkin->eatery->name); ?>

                                        <?php if($checkin->eatery->category): ?>
                                            <span style="font-weight: 500; opacity: 0.75;">• <?php echo e($checkin->eatery->category->name); ?></span>
                                        <?php endif; ?>
                                        <?php if($checkin->eatery->commune): ?>
                                            <span style="font-weight: 500; opacity: 0.75;">• <?php echo e($checkin->eatery->commune->name); ?></span>
                                        <?php endif; ?>
                                    </a>
                                <?php endif; ?>
                                ...
                                <?php if($checkin->comment): ?>
                                    <div class="post-comment">
                                        "<?php echo e($checkin->comment); ?>"
                                    </div>
                                <?php endif; ?>

                                <!-- Photo -->
                                <?php if($checkin->image_path): ?>
                                    <div class="post-photo-wrapper">
                                        <img src="<?php echo e($checkin->image_path); ?>" alt="Ảnh check-in" class="post-photo-img" loading="lazy"
                                            onclick="openStopImageModal('<?php echo e($checkin->image_path); ?>')">
                                    </div>
                                <?php endif; ?>
                                ...
                                <div class="reactions-bar">
                                    <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                                        ...
                                        <div class="reactions-list" data-post-id="<?php echo e($checkin->id); ?>" data-post-type="checkin">
                                            <?php
                                                $rCounts = $checkin->reaction_counts ?? ['❤️'=>0, '🔥'=>0, '👍'=>0, '😂'=>0, '😍'=>0, '🤤'=>0];
                                            ?>
                                            <button type="button" class="react-btn" data-emoji="❤️" onclick="sendReaction(<?php echo e($checkin->id); ?>, 'checkin', '❤️', event)">
                                                <span>❤️</span> <span class="react-count <?php echo e(($rCounts['❤️'] ?? 0) > 0 ? 'active-count' : ''); ?>"><?php echo e($rCounts['❤️'] ?? 0); ?></span>
                                            </button>
                                            <button type="button" class="react-btn" data-emoji="🔥" onclick="sendReaction(<?php echo e($checkin->id); ?>, 'checkin', '🔥', event)">
                                                <span>🔥</span> <span class="react-count <?php echo e(($rCounts['🔥'] ?? 0) > 0 ? 'active-count' : ''); ?>"><?php echo e($rCounts['🔥'] ?? 0); ?></span>
                                            </button>
                                            <button type="button" class="react-btn" data-emoji="👍" onclick="sendReaction(<?php echo e($checkin->id); ?>, 'checkin', '👍', event)">
                                                <span>👍</span> <span class="react-count <?php echo e(($rCounts['👍'] ?? 0) > 0 ? 'active-count' : ''); ?>"><?php echo e($rCounts['👍'] ?? 0); ?></span>
                                            </button>
                                    ...
                                    <div class="reaction-total-badge" style="<?php echo e(($checkin->reaction_total ?? 0) > 0 ? '' : 'display:none;'); ?>">
                                        <span class="total-num"><?php echo e($checkin->reaction_total ?? 0); ?></span> lượt bày tỏ
                                    </div>
                                </div>
                                ...
                                <div class="comments-section" style="margin-top: 15px; padding-top: 15px; border-top: 1px dashed var(--border-glow); display: flex; flex-direction: column; gap: 10px;">
                                    <h5 class="comments-count" style="margin: 0; font-size: 0.85rem; font-weight: 800; color: var(--text-muted); display: flex; align-items: center; gap: 6px;">💬 Bình luận (<?php echo e($checkin->comments->count()); ?>)</h5>
                                    <div class="comments-list" style="display: flex; flex-direction: column; gap: 8px;">
                                        <?php $__currentLoopData = $checkin->comments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <div class="comment-item" data-comment-id="<?php echo e($comment->id); ?>" style="display: flex; gap: 10px; align-items: flex-start; background: rgba(255, 255, 255, 0.02); border-radius: 12px; padding: 10px 14px; border: 1px solid rgba(255,255,255,0.03);">
                                                <div class="comment-avatar" style="width: 28px; height: 28px; border-radius: 50%; background: linear-gradient(135deg, rgba(var(--primary-rgb),0.1), rgba(16,185,129,0.08)); border: 1px solid rgba(var(--primary-rgb),0.2); display: flex; align-items: center; justify-content: center; font-size: 0.75rem; font-weight: 700; color: var(--text-main); flex-shrink: 0;">
                                                    <?php echo e($comment->user ? mb_substr($comment->user->name, 0, 1, 'UTF-8') : '👤'); ?>
                                                ...
                                                <div style="flex: 1; display: flex; flex-direction: column; gap: 3px;">
                                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                                        <span style="font-size: 0.82rem; font-weight: 700; color: var(--text-main);">
                                                            ...
                                                            <?php if($comment->user && $comment->user->role === 'admin'): ?>
                                                        ...
                                                        <span style="font-size: 0.7rem; color: var(--text-muted);"><?php echo e($comment->created_at->diffForHumans()); ?></span>
                                                    </div>
                                                    <p style="margin: 0; font-size: 0.85rem; color: var(--text-muted); line-height: 1.4;"><?php echo e($comment->content); ?></p>
                                                </div>
                                            </div>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    </div>
                                    ...
                                    <!-- Form bình luận -->
                                    <form action="<?php echo e(route('comments.store')); ?>" method="POST" style="display: flex; flex-direction: column; gap: 8px; margin-top: 5px;">
                                        <?php echo csrf_field(); ?>
                                        <input type="hidden" name="commentable_id" value="<?php echo e($checkin->id); ?>">
                ...
                <?php if($diaries->isNotEmpty()): ?>
                    <div id="diariesSection">
                        ...
                        <?php $__currentLoopData = $diaries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $diary): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <article class="checkin-post-card glass-panel" data-diary-id="<?php echo e($diary->id); ?>" data-user-id="<?php echo e($diary->user_id ?? ''); ?>" style="margin-bottom: 20px;">
                                <!-- Post Author Header -->
                                <div class="post-header">
                                    <div class="post-author">
                                        <div class="author-avatar">
                                            <?php echo e($diary->user->avatar ?? '👤'); ?>

                                        </div>
                                        <div class="author-meta">
                                            <span class="author-name"><?php echo e($diary->user->name ?? 'Thực khách Đông Anh'); ?></span>
                                            <span class="author-role-badge <?php echo e(($diary->user && $diary->user->role === 'admin') ? 'role-admin' : 'role-user'); ?>">
                                                <?php echo e(($diary->user && $diary->user->role === 'admin') ? 'Admin' : 'Thực khách'); ?>
                                    ...
                                    <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 6px;">
                                        <div class="rating-stars-container">
                                            <?php if($diary->rating): ?>
                                                <?php for($i = 1; $i <= 5; $i++): ?>
                                                    <span class="star-icon <?php echo e($i <= $diary->rating ? 'filled' : 'empty'); ?>">★</span>
                                                <?php endfor; ?>
                                            <?php else: ?>
                                                <span style="font-size: 0.8rem; color: var(--text-muted); font-style: italic;">Đã hoàn thành</span>
                                            <?php endif; ?>
                                        </div>
                                        <span class="post-time">📅 <?php echo e($diary->created_at->diffForHumans()); ?> (<?php echo e($diary->created_at->format('d/m/Y H:i')); ?>)</span>
                                    </div>
                                </div>

                                <?php if($diary->foodTour): ?>
                                    <a href="/food-tour/<?php echo e($diary->foodTour->slug); ?>" class="post-tour-link">
                                        🎯 Hành trình: <?php echo e($diary->foodTour->name); ?>
                                ...
                                <?php if($diary->image_path): ?>
                                    <div class="post-photo-wrapper">
                                        <img src="<?php echo e($diary->image_path); ?>" alt="Ảnh checkin của <?php echo e($diary->user->name ?? 'Thực khách'); ?>" class="post-photo-img" loading="lazy">
                                    </div>
                                <?php endif; ?>

                                <?php if(!empty($diary->stop_reviews) && $diary->foodTour): ?>
                                                ...
                                                <div class="timeline-node">
                                                    <div class="stop-detail-card">
                                                        <div class="stop-header">
                                                            <div class="stop-info">
                                                                <?php if($eatery): ?>
                                                                    <a href="/dia-diem/<?php echo e($eatery->slug); ?>" class="stop-name">
                                                                        <?php echo e($stopIdx + 1); ?>. <?php echo e($eatery->name); ?>

                                                                    </a>
                                                                    <span class="stop-category">
                                                                        🔖 <?php echo e($eatery->category->name ?? 'Ẩm thực'); ?> • 📍 <?php echo e($eatery->commune->name ?? 'Đông Anh'); ?>

                                                                    </span>
                                                                <?php else: ?>
                                                                    <span class="stop-name" style="color: var(--text-muted);">
                                                                        <?php echo e($stopIdx + 1); ?>. Điểm dừng chưa đồng bộ
                                                                    </span>
                                                                <?php endif; ?>
                                                            </div>
                                                            <?php if(!empty($stopRev['rating'])): ?>
                                                                <div class="rating-stars-container" style="scale: 0.9;">
                                                                    <?php for($i = 1; $i <= 5; $i++): ?>
                                                                        <span class="star-icon <?php echo e($i <= $stopRev['rating'] ? 'filled' : 'empty'); ?>">★</span>
                                                                    <?php endfor; ?>
                                                                </div>
                                                            <?php endif; ?>
                                                        </div>
                                                        <?php if(!empty($stopRev['comment'])): ?>
                                                            <p class="stop-comment">💭 <?php echo e($stopRev['comment']); ?></p>
                                                        <?php endif; ?>
                                                        <?php if(!empty($stopRev['image_path'])): ?>
                                                            <div class="stop-media-grid">
                                                                <img src="<?php echo e($stopRev['image_path']); ?>" alt="Ảnh chặng <?php echo e($stopIdx + 1); ?>" class="stop-media-thumb" onclick="openStopImageModal('<?php echo e($stopRev['image_path']); ?>')" loading="lazy">
                                                            </div>
                                                        <?php endif; ?>
                                                    </div>
                                                </div>
                                ...
                                <div class="reactions-bar">
                                    <div style="display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                                        <span style="font-size:0.8rem; font-weight:800; color:var(--text-muted);">Thả cảm xúc:</span>
                                        <div class="reactions-list" data-post-id="<?php echo e($diary->id); ?>" data-post-type="diary">
                                            <?php
                                                $rCounts = $diary->reaction_counts ?? ['❤️'=>0, '🔥'=>0, '👍'=>0, '😂'=>0, '😍'=>0, '🤤'=>0];
                                            ?>
                                            <button type="button" class="react-btn" data-emoji="❤️" onclick="sendReaction(<?php echo e($diary->id); ?>, 'diary', '❤️', event)">
                                                <span>❤️</span> <span class="react-count <?php echo e(($rCounts['❤️'] ?? 0) > 0 ? 'active-count' : ''); ?>"><?php echo e($rCounts['❤️'] ?? 0); ?></span>
                                            </button>
                                            <button type="button" class="react-btn" data-emoji="🔥" onclick="sendReaction(<?php echo e($diary->id); ?>, 'diary', '🔥', event)">
                                                <span>🔥</span> <span class="react-count <?php echo e(($rCounts['🔥'] ?? 0) > 0 ? 'active-count' : ''); ?>"><?php echo e($rCounts['🔥'] ?? 0); ?></span>
                                            </button>
                                            <button type="button" class="react-btn" data-emoji="👍" onclick="sendReaction(<?php echo e($diary->id); ?>, 'diary', '👍', event)">
                                                <span>👍</span> <span class="react-count <?php echo e(($rCounts['👍'] ?? 0) > 0 ? 'active-count' : ''); ?>"><?php echo e($rCounts['👍'] ?? 0); ?></span>
                                    ...
                                    <div class="reaction-total-badge" style="<?php echo e(($diary->reaction_total ?? 0) > 0 ? '' : 'display:none;'); ?>">
                                        <span class="total-num"><?php echo e($diary->reaction_total ?? 0); ?></span> lượt bày tỏ
                                    </div>
                                </div>

                                <!-- Comments Section -->
                                <div class="comments-section" style="margin-top: 15px; padding-top: 15px; border-top: 1px dashed var(--border-glow); display: flex; flex-direction: column; gap: 10px;">
                                    <h5 class="comments-count" style="margin: 0; font-size: 0.85rem; font-weight: 800; color: var(--text-muted); display: flex; align-items: center; gap: 6px;">💬 Bình luận (<?php echo e($diary->comments->count()); ?>)</h5>
                                    <div class="comments-list" style="display: flex; flex-direction: column; gap: 8px;">
                                        <?php $__currentLoopData = $diary->comments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <div class="comment-item" data-comment-id="<?php echo e($comment->id); ?>" style="display: flex; gap: 10px; align-items: flex-start; background: rgba(255, 255, 255, 0.02); border-radius: 12px; padding: 10px 14px; border: 1px solid rgba(255,255,255,0.03);">
                                                <div class="comment-avatar" style="width: 28px; height: 28px; border-radius: 50%; background: linear-gradient(135deg, rgba(var(--primary-rgb),0.1), rgba(16,185,129,0.08)); border: 1px solid rgba(var(--primary-rgb),0.2); display: flex; align-items: center; justify-content: center; font-size: 0.75rem; font-weight: 700; color: var(--text-main); flex-shrink: 0;">
                                                    <?php echo e($comment->user ? mb_substr($comment->user->name, 0, 1, 'UTF-8') : '👤'); ?>
                                    ...
                                    <form action="<?php echo e(route('comments.store')); ?>" method="POST" style="display: flex; flex-direction: column; gap: 8px; margin-top: 5px;">
                                        <?php echo csrf_field(); ?>
                                        <input type="hidden" name="commentable_id" value="<?php echo e($diary->id); ?>">
                ...
                <?php if($standaloneCheckins->isEmpty() && $diaries->isEmpty()): ?>
...
<?php $__env->startPush('realtime-scripts'); ?>
...
<script>
    ...
    window.Echo = new (class {
        constructor() {
            this._pusher = new Pusher('<?php echo e(env('REVERB_APP_KEY')); ?>', {
    ...
    console.log('[Reverb] Echo initialized — connecting to ws://<?php echo e(env('REVERB_HOST', 'localhost')); ?>:<?php echo e(env('REVERB_PORT', 8080)); ?>');
</script>
<?php $__env->stopPush(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\MNT\Documents\FOOD_MAP\resources\views/checkin.blade.php ENDPATH**/ ?>