...
<div class="admin-card">
    ...
    <form method="GET" action="/admin/dashboard" id="filterForm" onsubmit="event.preventDefault();" style="width: 100%; display: flex; gap: 12px; margin-bottom: 24px; align-items: center; flex-wrap: wrap; background-color: #f8fafc; padding: 14px; border-radius: 12px; border: 1px solid var(--admin-border);">
        <div style="flex: 2; min-width: 260px; position: relative;">
            <input type="text" name="q" id="eaterySearchInput" value="<?php echo e(request('q')); ?>" class="admin-form-input" placeholder="🔍 Tìm theo tên cơ sở, địa chỉ hoặc số điện thoại..." style="padding-left: 14px; width: 100%;">
        </div>
            ...
            <select name="category" id="categoryFilter" class="admin-form-input">
                ...
                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <option value="<?php echo e($cat->name); ?>" <?php echo e(request('category') == $cat->name ? 'selected' : ''); ?>>
                        <?php echo e($displayNameVi); ?> (<?php echo e($displayNameEn); ?>)
                    </option>
        ...
        <div style="flex: 1; min-width: 160px;">
            <select name="commune" id="communeFilter" class="admin-form-input">
                ...
                <?php $__currentLoopData = $communes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $com): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($com->name); ?>" <?php echo e(request('commune') == $com->name ? 'selected' : ''); ?>><?php echo e($com->name); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </div>
    ...
    <?php if($eateries->count() > 0): ?>
                ...
                <tbody>
                    <?php $__currentLoopData = $eateries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $eat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="eatery-table-row" 
                            ...
                            <td>
                                <img src="<?php echo e($eat->image_path ?: 'https://images.unsplash.com/photo-1591814468924-caf88d1232e1?auto=format&fit=crop&w=300&q=80'); ?>" style="width: 48px; height: 48px; object-fit: cover; border-radius: 8px; border: 1px solid var(--admin-border);" loading="lazy">
                            </td>
                            <td>
                            ...
                            <td>
                                <span style="font-size: 0.88rem; font-weight: 600; color: var(--admin-text-main);"><?php echo e($eat->commune->name); ?></span>
                            </td>
                            <td>
                            ...
                            <td>
                                <div style="font-size: 0.85rem; display: flex; flex-direction: column;">
                                    ...
                                    <span style="font-size: 0.72rem; color: var(--admin-text-muted);">(<?php echo e($eat->reviews_count); ?> reviews)</span>
                                </div>
                            </td>
                            <td style="text-align: center;">
                                <div style="display: inline-flex; gap: 6px; align-items: center;">
                                    <a href="/dia-diem/<?php echo e($eat->slug); ?>" target="_blank" class="btn-admin btn-admin-secondary" style="padding: 6px 12px; font-size: 0.75rem; border-radius: 6px;" title="Xem trang bản đồ khách hàng">
                                        Xem Map
                                    </a>
                                    
                                    <!-- Dynamic Purple CabaFood-style Quản Lý Button -->
                                    <a href="/admin/eateries/<?php echo e($eat->slug); ?>/edit" class="btn-admin btn-admin-primary" style="padding: 6px 14px; font-size: 0.78rem; border-radius: 6px; background-color: var(--admin-primary);">
                                        ⚙️ Quản lý
                                    </a>
                                    
                                    <?php if(session('user_role') === 'admin'): ?>
                                    <form action="/admin/eateries/<?php echo e($eat->slug); ?>" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa địa điểm này khỏi Bản đồ số Đông Anh không?')" style="display: inline;">
                                        <?php echo csrf_field(); ?>
        ...
        <?php if($eateries->hasPages()): ?>
            <div class="admin-pagination" style="display: flex; justify-content: center; align-items: center; gap: 8px; margin-top: 24px;">
                ...
                <?php else: ?>
                    <a href="<?php echo e($eateries->previousPageUrl()); ?>" style="padding: 8px 16px; border-radius: 8px; background: #ffffff; color: var(--admin-primary); border: 1px solid var(--admin-border); font-size: 0.85rem; font-weight: 600; text-decoration: none; transition: all 0.2s;" onmouseover="this.style.backgroundColor='var(--admin-primary-light)'" onmouseout="this.style.backgroundColor='#ffffff'">Trang trước</a>
                <?php endif; ?>
                ...
                <span style="font-size: 0.88rem; font-weight: bold; color: var(--admin-text-main); background: var(--admin-primary-light); padding: 8px 16px; border-radius: 8px; border: 1px solid rgba(79, 70, 229, 0.15);">
                    Trang <?php echo e($eateries->currentPage()); ?> / <?php echo e($eateries->lastPage()); ?>

                </span>

                
                <?php if($eateries->hasMorePages()): ?>
                    <a href="<?php echo e($eateries->nextPageUrl()); ?>" style="padding: 8px 16px; border-radius: 8px; background: #ffffff; color: var(--admin-primary); border: 1px solid var(--admin-border); font-size: 0.85rem; font-weight: 600; text-decoration: none; transition: all 0.2s;" onmouseover="this.style.backgroundColor='var(--admin-primary-light)'" onmouseout="this.style.backgroundColor='#ffffff'">Trang sau</a>
                <?php else: ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /www/wwwroot/donganhdiscovery.xadonganh.com/FOOD_MAP/resources/views/admin/dashboard.blade.php ENDPATH**/ ?>