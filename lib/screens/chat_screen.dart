...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  final Map<int, int> _previousUnreadMap = {};
  ...
  void _checkForNewUnreadMessages(List<dynamic> friends) {
    final isFirstLoad = _previousUnreadMap.isEmpty;
    for (final f in friends) {
      final id = f['id'];
      if (id == null) continue;
      final friendId = id is int ? id : int.tryParse(id.toString()) ?? 0;
      final currentUnread = _getUnreadCount(f);

      if (!isFirstLoad) {
        final previousUnread = _previousUnreadMap[friendId] ?? 0;
        if (currentUnread > previousUnread) {
          final name = (f['name'] ?? 'Bạn bè').toString();
          final msg = (f['latest_message'] ?? 'Tin nhắn mới').toString();
      ...
      _previousUnreadMap[friendId] = currentUnread;
    }
  }

  int _getUnreadCount(dynamic friend) {
    if (friend == null) return 0;
    if (friend['is_unread'] == true || friend['is_unread'] == 1 || friend['is_unread'] == '1' || friend['is_unread'] == 'true') return 1;
    if (friend['has_unread'] == true || friend['has_unread'] == 1) return 1;
    if (friend['is_read'] == false || friend['read'] == false || friend['is_read'] == 0 || friend['read'] == 0) return 1;
    final uc = friend['unread_count'] ?? friend['unread_messages'] ?? friend['unreads'];
    if (uc is int && uc > 0) return uc;
    if (uc != null) {
      final parsed = int.tryParse(uc.toString());
      if (parsed != null && parsed > 0) return parsed;
    }
    if (friend['unread'] == true || friend['unread'] == 'true' || friend['unread'] == 1 || friend['unread'] == '1' || friend['is_new'] == true) {
      return 1;
    }
    return 0;
  }
  ...
  void _sortFriendsList(List<dynamic> list) {
    list.sort((a, b) {
      final aUnread = _getUnreadCount(a);
      final bUnread = _getUnreadCount(b);

      final aHasUnread = aUnread > 0 ? 1 : 0;
      final bHasUnread = bUnread > 0 ? 1 : 0;

      if (aHasUnread != bHasUnread) {
        return bHasUnread.compareTo(aHasUnread); // Ưu tiên cuộc trò chuyện chưa đọc lên trên cùng
      }

      if (aUnread != bUnread) {
        return bUnread.compareTo(aUnread); // Ưu tiên số tin chưa đọc nhiều hơn
      }
  ...
  Future<void> _silentRefreshFriends() async {
    ...
    // Detect new unread messages and show local notification
    _checkForNewUnreadMessages(friends);
          ...
          : _friends.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final unreadCount = _getUnreadCount(friend);
                    ...
                    return Container(
                      decoration: BoxDecoration(
                        color: hasUnread ? const Color(0xFFF0F9FF) : Colors.white,
                        borderRadius: BorderRadius.circular(16),
                        border: Border.all(
                          color: hasUnread ? const Color(0xFF7DD3FC) : const Color(0xFFF1F5F9),
                      ...
                      child: ClipRRect(
                        ...
                        child: Stack(
                          children: [
                            ...
                            ListTile(
                              contentPadding: EdgeInsets.fromLTRB(hasUnread ? 16 : 14, 6, 10, 6),
                              leading: Stack(
                                  ...
                                  Container(
                                    ...
                                    child: CircleAvatar(
                                      ...
                                      backgroundImage: ResizeImage(
                                        NetworkImage(ApiService.getAvatarUrl(friend, friend['name'])),
                                        width: 120,
                                      ),
                                    ),
                                  ),
                              ...
                              subtitle: Padding(
                                ...
                                child: Row(
                                  children: [
                                    if (hasUnread) ...[
                                      const Icon(Icons.circle, size: 7, color: Color(0xFF0EA5E9)),
