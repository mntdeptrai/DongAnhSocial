...
<div class="market-container">
    ...
    <div class="top-db-row">
        ...
        <div class="premium-panel premium-panel-metric premium-panel-success" data-aos="fade-up" data-aos-delay="150" style="margin-bottom:0;">
            ...
            <p style="font-size: 0.78rem; color: var(--text-muted); margin: 0;">Tỷ lệ phủ mã VietQR đạt <strong>{{ $qrPercentage }}%</strong>.</p>
        </div>
        ...
        <div class="premium-panel premium-panel-metric" data-aos="fade-up" data-aos-delay="250" style="margin-bottom:0;">
            ...
            <p style="font-size: 0.78rem; color: var(--text-muted); margin: 0;">Tỷ lệ sử dụng thiết bị thông minh đạt <strong>{{ $phonePercentage }}%</strong>.</p>
        </div>
                ...
                <div class="stall-card-wrapper" 
                    ...
                    <div class="stall-card-gov">
                        ...
                        <div class="btn-stall-grid">
                            ...
                            <button onclick="openStallDetail('{{ $stallName }}', '{{ $sellerName }}', '{{ $sellerPhone }}', '{{ $bankInfo }}', '{{ $category }}', '{{ $originText }}', '{{ $hasSmartphone ? 'yes' : 'no' }}', {{ json_encode($stallProducts) }}, '{{ $first->latitude ?? '' }}', '{{ $first->longitude ?? '' }}')" class="btn-stall-action">
...
<script>
    ...
    function initSatelliteMap() {
        ...
        L.marker([eateryLat, eateryLng], { icon: customIcon })
            .addTo(mainMap)
            .bindPopup(`<strong style="font-family: var(--font-heading); font-size:1rem;">{{ $eatery->name }}</strong><br>📍 ${eateryLat}, ${eateryLng}<br>🏪 Quy mô: {{ $totalStalls }} hộ kinh doanh`)
            .openPopup();
