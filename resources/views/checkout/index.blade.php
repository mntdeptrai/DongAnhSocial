                        ...
                        <div style="display: flex; flex-direction: column; gap: 8px; max-height: 150px; overflow-y: auto; padding-right: 4px;">
                            @foreach($vouchers as $voucher)
                                <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; border-radius: 8px; background: rgba(255, 126, 41, 0.04); border: 1px dashed rgba(255, 126, 41, 0.25); font-size: 0.8rem;">
                                    <div>
                                        <strong style="color: var(--primary); font-size: 0.85rem;">{{ $voucher->code }}</strong>
                                        <span style="color: var(--text-muted); display: block; font-size: 0.72rem;">Giảm {{ $voucher->percentage }}% cho đơn từ {{ number_format($voucher->min_order_amount) }}đ</span>
                                    </div>
                                    <button type="button" onclick="selectVoucher('{{ $voucher->code }}')" style="border: none; background: var(--primary-grad); color: white; padding: 3px 10px; border-radius: 6px; font-size: 0.7rem; font-weight: 700; cursor: pointer;">
                                        Chọn
                                    </button>
                                </div>
                            @endforeach
