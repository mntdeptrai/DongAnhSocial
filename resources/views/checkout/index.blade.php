    ...
    <div style="display: grid; grid-template-columns: 1.2fr 1fr; gap: 30px; align-items: start;">
        ...
        <div style="display: flex; flex-direction: column; gap: 20px;">
                ...
                <div style="border-top: 1px dashed var(--border-glow); padding-top: 16px; display: flex; flex-direction: column; gap: 10px; font-size: 0.9rem;">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; color: #2ecc71;">
                        ...
                        <strong id="checkoutDiscount">
                            @if($bestVoucherApplied)
                                -{{ $bestVoucherApplied['discount'] }}
                            @else
                                0đ
                            @endif
                        </strong>
            ...
            <div class="glass-panel" style="padding: 24px; border-radius: 20px; border: 1px solid var(--border-glow); background: rgba(255,255,255,0.015);">
                ...
                <div id="voucherStatusBanner" style="{{ $bestVoucherApplied ? 'display: flex;' : 'display: none;' }} justify-content: space-between; align-items: center; background: rgba(46, 204, 113, 0.1); border: 1.5px solid rgba(46, 204, 113, 0.25); border-radius: 12px; padding: 12px 16px; margin-bottom: 18px; font-size: 0.85rem;">
                    <div style="display: flex; align-items: center; gap: 8px; color: #27ae60; font-weight: 600;">
                        <span style="font-size: 1rem;">✅</span>
                        <span id="bannerTextSpan">
                            @if($bestVoucherApplied)
                                Đã tự động áp dụng mã <strong>{{ $bestVoucherApplied['code'] }}</strong> (giảm {{ (int)$bestVoucherApplied['percent'] }}%)
                            @endif
                        </span>
                    </div>
                ...
                <div style="display: flex; gap: 10px; margin-bottom: 20px;">
                    <input type="text" id="voucherCodeInput" placeholder="Nhập mã ưu đãi..." value="{{ $bestVoucherApplied ? $bestVoucherApplied['code'] : '' }}" style="flex: 1; padding: 10px 14px; border-radius: 10px; border: 1.5px solid var(--border-glow); background: rgba(255,255,255,0.02); color: var(--text-main); outline: none; font-size: 0.88rem;">
                    ...
                    <div style="display: flex; flex-direction: column;">
                        ...
                        <div id="vouchersListWrapper" style="display: flex; flex-direction: column; gap: 10px; max-height: 250px; overflow-y: auto; padding-right: 4px;">
                            @foreach($vouchers as $voucher)
                                ...
                                @endphp
                                <div class="voucher-card-item {{ $isApplied ? 'applied' : ($isEligible ? 'eligible' : 'ineligible') }}" 
                                     data-code="{{ $voucher->code }}" 
                                     data-eligible="{{ $isEligible ? 'true' : 'false' }}"
                                     style="display: flex; align-items: center; border: 1.5px solid {{ $isApplied ? '#2ecc71' : 'var(--border-glow)' }}; border-radius: 12px; background: {{ $isApplied ? 'rgba(46, 204, 113, 0.03)' : 'rgba(255,255,255,0.01)' }}; padding: 12px; position: relative; overflow: hidden; gap: 12px; transition: all 0.25s; opacity: {{ $isEligible ? '1' : '0.65' }};">
                                    ...
                                    <div style="width: 60px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; flex-shrink: 0; z-index: 2;">
                                        ...
                                        <span style="font-size: 0.85rem; font-weight: 800; color: var(--primary);">{{ (int)$voucher->percentage }}%</span>
                                    </div>
                                    ...
                                    <div style="flex: 1; padding-left: 12px; min-width: 0; z-index: 2; display: flex; flex-direction: column; gap: 3px;">
                                        <div style="display: flex; align-items: center; gap: 6px;">
                                            <strong style="font-size: 0.9rem; color: var(--text-main); font-family: var(--font-heading);">{{ $voucher->code }}</strong>
                                            <span class="badge-applied" style="background: rgba(46, 204, 113, 0.15); color: #2ecc71; font-size: 0.65rem; font-weight: 700; padding: 1px 6px; border-radius: 4px; display: {{ $isApplied ? 'inline-block' : 'none' }};">Đang dùng</span>
                                        </div>
                                        <span style="font-size: 0.75rem; color: var(--text-muted); display: block; font-weight: 500;">
                                            Giảm {{ (int)$voucher->percentage }}% cho đơn từ {{ number_format($voucher->min_order_amount) }}đ
                                        </span>
                                        @if(!$isEligible)
                                            <span style="font-size: 0.7rem; color: #e74c3c; font-weight: 600; display: flex; align-items: center; gap: 3px;">
                                                ⚠️ Chưa đủ điều kiện (Thiếu {{ number_format($voucher->min_order_amount - $subtotal) }}đ)
                                            </span>
                                        @endif
                                    ...
                                    <div style="z-index: 2; flex-shrink: 0;">
                                        ...
                                        @elseif($isEligible)
                                            <button type="button" onclick="selectVoucher('{{ $voucher->code }}')" class="btn-voucher-action" style="border: none; background: var(--primary-grad); color: white; padding: 6px 12px; border-radius: 8px; font-size: 0.75rem; font-weight: 700; cursor: pointer; transition: all 0.2s; box-shadow: 0 4px 10px rgba(255,126,41,0.2);">
                                                Áp dụng
                                            </button>
                                        @else
