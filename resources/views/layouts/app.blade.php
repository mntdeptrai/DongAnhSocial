                ...
                <div x-show="!chat.is_minimized" class="fchat-window" style="height: 520px;">
                    ...
                    <div class="fchat-header" @click="chat.is_minimized = !chat.is_minimized">
                        ...
                        <div style="display: flex; align-items: center; gap: 3px; flex-shrink: 0;" @click.stop.prevent>
                            ...
                            <button type="button" class="fchat-header-btn" title="Gọi thoại" @click.stop.prevent="if (window.DongAnhWebRTC) { window.DongAnhWebRTC.startCall(chat.id, chat.name, chat.avatar_url || chat.avatar, 'audio'); } else { alert('Hệ thống gọi thoại đang sẵn sàng...'); }">
                            ...
                            <button type="button" class="fchat-header-btn" title="Gọi video" @click.stop.prevent="if (window.DongAnhWebRTC) { window.DongAnhWebRTC.startCall(chat.id, chat.name, chat.avatar_url || chat.avatar, 'video'); } else { alert('Hệ thống gọi video đang sẵn sàng...'); }">
