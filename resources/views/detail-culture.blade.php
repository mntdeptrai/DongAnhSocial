...
<div class="container" style="padding-top: 20px; margin-bottom: 24px;">
    ...
    <div class="detail-header-wrapper">
        ...
        <div class="detail-header-actions">
            ...
            <button id="btnSaveLocCulture" onclick="toggleSaveLocation(this, '{{ $eatery->slug }}', '{{ addslashes($eatery->name) }}')" class="btn-secondary" style="border-radius: 30px; padding: 8px 18px; font-size: 0.9rem; border-color: var(--border-glow); display: flex; align-items: center; gap: 6px; background: var(--bg-card); color: var(--text-main); font-weight: 600; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='none'">
                <span class="save-icon" style="font-size: 1.1rem;">❤️</span> <span class="save-text">Lưu lại</span>
            </button>
        </div>
        <script>document.addEventListener('DOMContentLoaded', () => initSaveButtonState(document.getElementById('btnSaveLocCulture'), '{{ $eatery->slug }}'));</script>
    </div>
</div>
...
<div class="container">
    <div class="detail-grid">
        ...
        <div>
            ...
            @php
                $items = $eatery->culturalActivities ?? collect();
                $title = 'Góc Trải Nghiệm & Hoạt Động Văn Hóa - Thể Thao';
                $icon = '⛩️';
                $emptyText = 'Chưa cập nhật hoạt động di sản hay trải nghiệm văn hóa.';
                $btnText = 'Xem tất cả hoạt động';
                $modalTitle = 'Hoạt động Di sản & Văn hóa - ' . $eatery->name;
                $itemUnit = 'hoạt động';
                $placeholderSearch = 'Tìm hoạt động trải nghiệm, vé, dịch vụ...';
            @endphp
