...
<style>
    :root {
        ...
        --category-accent-dark: #0f766e;
        --category-glow: rgba(13, 148, 136, 0.15);
    }
</style>
    ...
    <div class="detail-header-wrapper">
        <div class="detail-header-left">
            <div class="detail-header-badges">
                <span class="tag-badge-accent" style="display: inline-flex; align-items: center; gap: 6px; font-size: 0.85rem; color: #0d9488; background: rgba(13, 148, 136, 0.1); border: 1px solid rgba(13, 148, 136, 0.3); backdrop-filter: blur(8px); font-weight: 800; border-radius: 30px; padding: 6px 14px;">
                    🩺 Wellness & Care - Y Tế, Bệnh Viện & Spa Thư Giãn
                </span>
                <span class="tag-badge-accent" style="display: inline-flex; align-items: center; gap: 6px; font-size: 0.85rem; color: #0284c7; background: rgba(2, 132, 199, 0.1); border: 1px solid rgba(2, 132, 199, 0.3); font-weight: 700; border-radius: 30px; padding: 6px 14px;">
                    🩺 Đạt chuẩn Giấy phép Y tế
                </span>
        ...
        <div class="detail-header-actions">
            ...
            <button id="btnSaveLocWellness" onclick="toggleSaveLocation(this, '{{ $eatery->slug }}', '{{ addslashes($eatery->name) }}')" class="btn-secondary" style="border-radius: 30px; padding: 8px 18px; font-size: 0.9rem; border-color: var(--border-glow); display: flex; align-items: center; gap: 6px; background: var(--bg-card); color: var(--text-main); font-weight: 600; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='none'">
                <span class="save-icon" style="font-size: 1.1rem;">❤️</span> <span class="save-text">Lưu lại</span>
            </button>
        </div>
        <script>document.addEventListener('DOMContentLoaded', () => initSaveButtonState(document.getElementById('btnSaveLocWellness'), '{{ $eatery->slug }}'));</script>
    </div>
</div>
    ...
    <div class="detail-grid">
        ...
        <div>
            ...
            @php
                ...
                $title = 'Dịch vụ Y tế & Chăm sóc sức khỏe - Spa';
                $icon = '🩺';
                $emptyText = 'Chưa cập nhật dịch vụ y tế cho cơ sở này.';
                $btnText = 'Xem tất cả gói dịch vụ';
                $modalTitle = 'Dịch vụ Y tế & Chăm sóc - ' . $eatery->name;
                $itemUnit = 'dịch vụ';
                $placeholderSearch = 'Tìm dịch vụ chăm sóc, khám chữa bệnh, spa...';
            @endphp
