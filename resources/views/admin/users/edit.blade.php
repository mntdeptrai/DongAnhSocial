    ...
    <form action="/admin/users/{{ $user->id }}" method="POST" style="padding: 24px 0 8px 0;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
            ...
            <div class="admin-form-group">
                ...
                <div style="position: relative;">
                    <span style="position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--admin-text-muted);">🔑</span>
                    <input type="text" name="username" required value="{{ old('username', $user->username) }}" class="admin-form-input" pattern="^[a-zA-Z0-9_.-]+$" title="Tên đăng nhập chỉ gồm chữ, số, dấu gạch nối, gạch dưới hoặc dấu chấm (không dấu, không khoảng trắng)" placeholder="Nhập tên đăng nhập (Ví dụ: nguyenvana)..." style="padding-left: 38px; border-radius: 10px; height: 42px;">
                </div>
            </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 24px;">
            ...
            <!-- Thay đổi mật khẩu -->
            <div class="admin-form-group">
                <label class="admin-form-label" style="font-weight: 700; font-size: 0.82rem; margin-bottom: 8px; display: block; color: var(--admin-text-main);">Thay đổi mật khẩu <span style="color: var(--admin-text-muted); font-weight: normal;">(Để trống nếu giữ nguyên)</span></label>
                <div style="position: relative;">
                    <span style="position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--admin-text-muted);">🔒</span>
        ...
        <div id="stall_selection_group" class="admin-form-group" style="margin-bottom: 24px;">
            ...
            <div style="position: relative;">
                <span style="position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--admin-text-muted);">🛒</span>
                <select name="stall_id" id="stall_id_select" class="admin-form-input" style="padding-left: 38px; border-radius: 10px; height: 42px; font-size: 0.86rem;">
                    <option value="">-- Chọn Gian hàng Chợ số quản lý --</option>
                    @php
                    ...
                    @foreach($stallsGrouped as $mId => $mStalls)
                        ...
                        <optgroup label="🏛️ {{ $mName }}">
                            @foreach($mStalls as $st)
                                <option value="{{ $st->id }}" {{ old('stall_id', $currentStallId ?? null) == $st->id ? 'selected' : '' }}>
            ...
            <div style="position: relative;">
                ...
                <select name="eatery_id" id="eatery_id_select" class="admin-form-input" style="padding-left: 38px; border-radius: 10px; height: 42px; font-size: 0.86rem;">
                    ...
                    @foreach($eateries as $eat)
                        <option value="{{ $eat->id }}" data-category="{{ $eat->category ? $eat->category->slug : '' }}" {{ old('eatery_id', $currentEateryId) == $eat->id ? 'selected' : '' }}>
