...
<div x-data="{ 
    ...
    submitPlanner() {
        ...
        fetch('/api/food-tours/generate-ai', {
            ...
            .then(data => {
                if (data.success) {
                    if (typeof window.showToast === 'function') {
                        window.showToast('✨ Đã khởi tạo thành công Lộ trình AI cho bạn!', 'success');
                    }
                    setTimeout(() => {
            ...
            .catch(err => {
                if (typeof window.showToast === 'function') {
                    window.showToast('Lỗi kết nối máy chủ! Vui lòng thử lại sau.', 'error');
                } else {
                        ...
                        <div class="tour-card hover-lift reveal reveal-scale-in" x-show="activeMood === '' || activeMood === '{{ $tour->mood }}'" data-mood="{{ $tour->mood }}" style="position: relative;">
                                ...
                                <div class="tour-card-body">
                                    ...
                                    <div style="display: flex; gap: 8px;">
                                        <a href="/food-tour/{{ $tour->slug }}" class="btn-primary" style="flex: 1.4; text-align: center; text-decoration: none; padding: 12px; border-radius: 12px; font-weight: 700; display: flex; align-items: center; justify-content: center; box-shadow: var(--shadow-glow); font-size: 0.85rem;">
                                            🚀 Trải nghiệm
                                        </a>
                                        @if($tour->diaries_count > 0)
                                        <button type="button" onclick="openDiariesModal('{{ $tour->id }}')" class="btn-secondary" style="flex: 1.2; text-align: center; padding: 12px; border-radius: 12px; font-weight: 700; background: rgba(255,255,255,0.05); color: var(--text-main); border: 1px solid var(--border-glow); display: flex; align-items: center; justify-content: center; gap: 4px; font-size: 0.85rem; cursor: pointer;">
                                            📖 Nhật ký ({{ $tour->diaries_count }})
                                        </button>
                                        @endif
                                        @if($canEdit && !$tour->is_ai_generated)
                                        <a href="/food-tour/{{ $tour->slug }}/edit" class="btn-secondary" title="Chỉnh sửa lộ trình thủ công" style="padding: 12px 14px; border-radius: 12px; font-weight: 700; background: rgba(255,255,255,0.05); color: var(--text-main); border: 1px solid var(--border-glow); display: flex; align-items: center; justify-content: center; font-size: 0.85rem; text-decoration: none;">
                                            ✏️
                                        </a>
                                        @endif
                                    </div>
        ...
        <div id="communityDiariesModal-{{ $tourData->id }}" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.4); backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); z-index: 10005; align-items: center; justify-content: center; animation: fadeIn 0.3s ease;">
            <div style="width: 90%; max-width: 650px; max-height: 85vh; display: flex; flex-direction: column; padding: 30px; border-radius: 24px; background: #ffffff; border: 1px solid #e2e8f0; box-shadow: 0 25px 50px -12px rgba(15, 23, 42, 0.25); color: #0f172a; position: relative;">
                ...
                <h3 style="font-weight: 800; color: #0f172a; font-size: 1.4rem; margin: 0 0 20px 0; display: flex; align-items: center; gap: 12px; border-bottom: 1px solid #f1f5f9; padding-bottom: 16px;">
                    📖 Nhật ký Cộng đồng
                    <span style="font-size: 0.8rem; background: #e0f2fe; border: 1px solid #bae6fd; color: #0284c7; padding: 4px 12px; border-radius: 20px; font-weight: 800;">{{ $tourData->diaries_count }} đánh giá</span>
                </h3>
