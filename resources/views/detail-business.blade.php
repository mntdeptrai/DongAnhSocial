...
<!-- Tối ưu hóa SEO: Tiêu đề động cho Cơ sở kinh doanh -->
@section('title', $eatery->name . ' - Cơ sở kinh doanh tại ' . ($eatery->commune?->name ?? 'Đông Anh') . ', Đông Anh')

<!-- Tối ưu hóa SEO: Thẻ mô tả Meta tự sinh chân thực -->
@section('meta_description', 'Khám phá sản phẩm & hàng hóa dịch vụ tại ' . $eatery->name . ', địa chỉ: ' . $eatery->address . ', ' . ($eatery->commune?->name ?? 'Đông Anh') . ', Đông Anh. Xem danh mục sản phẩm, đặt mua trực tiếp và hỗ trợ thanh toán VietQR.')
...
<div class="container" style="padding-top: 20px; margin-bottom: 24px;">
    ...
    <div class="detail-header-wrapper">
        ...
        <div class="detail-header-actions">
            ...
            <button id="btnSaveLocOcop" onclick="toggleSaveLocation(this, '{{ $eatery->slug }}', '{{ addslashes($eatery->name) }}')" class="btn-secondary" style="border-radius: 30px; padding: 8px 18px; font-size: 0.9rem; border-color: var(--border-glow); display: flex; align-items: center; gap: 6px; background: var(--bg-card); color: var(--text-main); font-weight: 600; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='none'">
                <span class="save-icon" style="font-size: 1.1rem;">❤️</span> <span class="save-text">Lưu lại</span>
            </button>
        </div>
        <script>document.addEventListener('DOMContentLoaded', () => initSaveButtonState(document.getElementById('btnSaveLocOcop'), '{{ $eatery->slug }}'));</script>
    </div>
</div>
                    ...
                    <div style="position: relative; z-index: 2;">
                        ...
                        <div class="heritage-tabs-container">
                            ...
                            <div id="tab-ingredients" class="heritage-tab-content">
                                ...
                                @php
                                    $ingList = $dossier['ingredients'] ?? [];
                                    if (is_string($ingList)) {
                                        $rawIngStr = str_replace(['\r\n', '\n', '\r'], "\n", $ingList);
                                        $ingList = array_values(array_filter(array_map('trim', explode("\n", str_replace("\r", "", $rawIngStr)))));
                                    }
                                @endphp
                                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 14px;">
                                    @forelse($ingList as $ingredient)
                                        <div class="glass-panel" style="padding: 14px 20px; background: rgba(212, 175, 55, 0.03); border: 1px solid rgba(212, 175, 55, 0.15); display: flex; align-items: center; gap: 12px;">
                                            ...
                                            <span style="font-size: 0.95rem; font-weight: 600; color: var(--text-main);">{{ is_array($ingredient) ? ($ingredient['name'] ?? json_encode($ingredient)) : $ingredient }}</span>
                                        </div>
                                    @empty
                                        <div style="color: var(--text-muted); font-style: italic; padding: 12px;">Đang cập nhật thành phần...</div>
                                    @endforelse
                                </div>
                            </div>
                            ...
                            <div id="tab-timeline" class="heritage-tab-content">
                                @php
                                    $timeList = $dossier['timeline'] ?? [];
                                    if (is_string($timeList)) {
                                        $rawTimeStr = str_replace(['\r\n', '\n', '\r'], "\n", $timeList);
                                        $lines = array_values(array_filter(array_map('trim', explode("\n", str_replace("\r", "", $rawTimeStr)))));
                                        $timeList = [];
                                        foreach ($lines as $line) {
                                            $parts = explode('|', $line, 2);
                                            if (count($parts) === 2) {
                                                $timeList[] = ['year' => trim($parts[0]), 'event' => trim($parts[1])];
                                            } else {
                                                $timeList[] = ['year' => 'Di sản', 'event' => trim($line)];
                                            }
                                        }
                                    }
                                @endphp
                                <div class="heritage-timeline">
                                    @forelse($timeList as $item)
                                        @php
                                            $y = is_array($item) ? ($item['year'] ?? 'Mốc di sản') : 'Mốc di sản';
                                            $e = is_array($item) ? ($item['event'] ?? $item) : $item;
                                        ...
                                        <div class="heritage-timeline-item">
                                            <div class="heritage-timeline-badge">{{ $y }}</div>
                                            <div class="heritage-timeline-content glass-panel" style="margin-left: 20px;">
                                                <p style="font-size: 0.95rem; margin: 0; line-height: 1.6; color: var(--text-main);">{{ $e }}</p>
                                            </div>
                                        </div>
                                    @empty
                                        <div style="color: var(--text-muted); font-style: italic; padding: 12px;">Đang cập nhật hành trình di sản...</div>
                                    @endforelse
                                </div>
                            </div>
                        </div>
                                ...
                                @foreach($items as $item)
                                    @if(in_array($categorySlug, ['hanh-trinh-di-san', 'discover-dong-anh-community-culture-hub']))
                                        <div class="dish-card glass-panel" style="background: rgba(255, 255, 255, 0.02); flex: 0 0 350px; min-width: 320px; display: flex; flex-direction: column; gap: 0; padding: 0; overflow: hidden; border: 1.5px solid rgba(16, 185, 129, 0.2); transition: all 0.3s ease; border-radius: 16px;">
                                            <div style="position: relative; height: 160px; overflow: hidden; width: 100%;">
                                                <img src="{{ $item->image_path ?: asset('images/ocop-placeholder.png') }}" style="width: 100%; height: 100%; object-fit: cover; transition: transform 0.5s ease;" alt="{{ $item->name }}">
                                    ...
                                    @else
                                        <div class="dish-card glass-panel" 
                                            ...
                                            <img src="{{ $item->image_path ?: ($categorySlug === 'smart-education-map' ? 'https://images.unsplash.com/photo-1546410531-bb4caa6b424d?auto=format&fit=crop&w=150&q=80' : ($categorySlug === 'wellness-care' ? 'https://images.unsplash.com/photo-1505751172876-fa1923c5c528?auto=format&fit=crop&w=150&q=80' : ($categorySlug === 'stay-in-dong-anh' ? 'https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=150&q=80' : ($categorySlug === 'dong-anh-market' ? 'https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&w=150&q=80' : asset('images/ocop-placeholder.png')))) }}" class="dish-img" alt="{{ $item->name }}">
                            ...
                            @foreach($items as $item)
                                @if(in_array($categorySlug, ['hanh-trinh-di-san', 'discover-dong-anh-community-culture-hub']))
                                    <div class="dish-card glass-panel" 
                                        ...
                                        <div style="position: relative; height: 160px; overflow: hidden; width: 100%;">
                                            <img src="{{ $item->image_path ?: asset('images/ocop-placeholder.png') }}" style="width: 100%; height: 100%; object-fit: cover;" alt="{{ $item->name }}">
                                            <div style="position: absolute; top: 12px; left: 12px; display: flex; flex-direction: column; gap: 6px; z-index: 2;">
                                                @if($item->type === 'experience')
                                ...
                                @else
                                    <div class="dish-card glass-panel" 
                                        ...
                                        <img src="{{ $item->image_path ?: ($categorySlug === 'smart-education-map' ? 'https://images.unsplash.com/photo-1546410531-bb4caa6b424d?auto=format&fit=crop&w=150&q=80' : ($categorySlug === 'wellness-care' ? 'https://images.unsplash.com/photo-1505751172876-fa1923c5c528?auto=format&fit=crop&w=150&q=80' : ($categorySlug === 'stay-in-dong-anh' ? 'https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=150&q=80' : ($categorySlug === 'dong-anh-market' ? 'https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&w=150&q=80' : asset('images/ocop-placeholder.png')))) }}" style="width: 80px; height: 80px; object-fit: cover; border-radius: 8px; flex-shrink: 0;" alt="{{ $item->name }}">
