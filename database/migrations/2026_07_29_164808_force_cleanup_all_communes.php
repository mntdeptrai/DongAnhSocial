...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // Tự động gán ID duy nhất từ 1 đến 36 để đồng bộ hoàn toàn giữa các connections
        $idCounter = 1;
        ...
        foreach ($connections as $conn) {
            ...
            $db->transaction(function() use ($db, $mapping) {
                // 1. Sao lưu bảng hiện tại nếu chưa có
                ...
                // Tắt kiểm tra khóa ngoại để tránh lỗi khi truncate communes
                Schema::connection($db->getName())->disableForeignKeyConstraints();

                // 2. Xóa sạch bảng communes hiện tại để tránh lỗi trùng Unique Slug trong lúc update
                $db->table('communes')->delete();

                // 3. Chèn lại 36 communes mới chuẩn với đúng ID từ 1 đến 36
                foreach ($mapping as $group) {
                ...
                // 4. Ánh xạ lại eateries dựa trên bảng backup communes_backup_72
                $backupCommunes = $db->table('communes_backup_72')->get();
                $backupEateries = $db->table('eateries_backup_2026')->get();

                foreach ($backupEateries as $eat) {
                    $oldCommune = $backupCommunes->firstWhere('id', $eat->commune_id);
                    if (!$oldCommune) {
                        continue;
                    }

                    $oldSlug = $oldCommune->slug;
                    $targetId = null;

                    // Tìm ID thôn mới tương ứng
                    foreach ($mapping as $group) {
                        $searchSlugs = array_merge($group['old_slugs'], [$group['slug']]);
                        if (in_array($oldSlug, $searchSlugs)) {
                            $targetId = $group['id'];
                            break;
                        }
                    }
                ...
                Schema::connection($db->getName())->enableForeignKeyConstraints();
            });
        }
    }
    ...
    {
        // Không cần rollback thủ công do đã có backup đầy đủ
    }
};
