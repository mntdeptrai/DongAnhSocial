...
return new class extends Migration
{
    ...
    {
        $marketId = 26; // Chợ Lực Canh

        // 1. Cập nhật thông tin Chợ Lực Canh trong eateries
        DB::table('eateries')->where('id', $marketId)->update([
            'name' => 'Chợ Lực Canh',
            'category_id' => 8, // Chợ truyền thống
            'address' => 'Thôn Lực Canh, Xã Xuân Canh, Huyện Đông Anh, Hà Nội',
            'updated_at' => now(),
        ]);

        // 2. Tạo / Cập nhật tài khoản Ban Quản lý Chợ Lực Canh
        $bql = User::where('email', 'bql.choluccanh@foodmap.vn')->orWhere('email', 'bql.choLucCanh@foodmap.vn')->first();
        if (!$bql) {
            DB::table('users')->insert([
                'name' => 'Ban Quản lý Chợ Lực Canh',
                'email' => 'bql.choluccanh@foodmap.vn',
                'phone' => '0123654026',
        ...
        $sellerEmailsToKeep = ['bql.choluccanh@foodmap.vn'];
        for ($i = 1; $i <= 10; $i++) {
            $sellerEmailsToKeep[] = "seller.lucCanh.{$i}@foodmap.vn";
        }
  ...
  array (
    ...
    'qr_bin' => '970436',
        ...
        // Xóa toàn bộ sạp cũ thuộc Chợ Lực Canh để nạp lại chuẩn xác 10 sạp
        DB::table('ocop_products')->where('eatery_id', $marketId)->delete();

        foreach ($merchants as $stt => $m) {
            $stallId = 3000 + $stt;
            $email = "seller.lucCanh.{$stt}@foodmap.vn";
            $phone = $m['phone'] ?: '';
            $name = $m['name'];
            ...
            // Xử lý tạo Gian hàng
            $desc = "Nguồn gốc: {$m['origin']}. Hộ kinh doanh tại Chợ Lực Canh, Xuân Canh.";
